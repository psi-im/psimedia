...
void expectMode(const QString &nativeCaps, const QString &expectedMime, int width, int height, int fpsNum, int fpsDen)
{
    ...
    int actualFpsNum = 0;
    int actualFpsDen = 0;
    if (!gst_structure_get_int(s, "width", &actualWidth)
        || !gst_structure_get_int(s, "height", &actualHeight)
...
int main(int argc, char **argv)
{
    ...
    expectMode(QStringLiteral(
                   "video/x-raw,width=(int)[320,1920],height=(int)[240,1080],framerate=(fraction)[5/1,30/1]"),
