...
static const GstElementDetails gst_speex_dsp_details
    = GST_ELEMENT_DETAILS("Voice processor", "Generic/Audio", "Preprepocesses voice with libspeexdsp",
...
static void gst_speex_dsp_finalize(GObject *object);
static void gst_speex_dsp_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gst_speex_dsp_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static GstStateChangeReturn gst_speex_dsp_change_state(GstElement *element, GstStateChange transition);

static gboolean gst_speex_dsp_setcaps(GstPad *pad, GstCaps *caps);
static GstCaps *gst_speex_dsp_getcaps(GstPad *pad);

static GstFlowReturn gst_speex_dsp_rec_chain(GstPad *pad, GstBuffer *buffer);
static gboolean      gst_speex_dsp_rec_event(GstPad *pad, GstEvent *event);

static const GstQueryType *gst_speex_dsp_query_type(GstPad *pad);
static gboolean            gst_speex_dsp_query(GstPad *pad, GstQuery *query);

static void gst_speex_dsp_reset_locked(GstSpeexDSP *self);

static void try_auto_attach();

static GstBuffer *try_echo_cancel(SpeexEchoState *echostate, const GstBuffer *recbuf, GstClockTime rec_adj,
                                  GstClockTime rec_base, GQueue *buffers, int rate, GstPad *srcpad,
                                  const GstCaps *outcaps, GstFlowReturn *res, GstSpeexDSP *obj);

static FileLog *filelog_new(const char *fname)
{
    FileLog *fl;
    FILE *   fp;
...
static void filelog_delete(FileLog *fl)
{
    fclose(fl->fp);
    free(fl);
}

static void filelog_append(FileLog *fl, const unsigned char *buf, int offset, int size)
{
    int n, pad, start, len;
...
static PairLog *pairlog_new(const char *pfname, const char *cfname)
{
    PairLog *pl;
...
static void pairlog_delete(PairLog *pl)
{
    if (pl->playback)
...
static void pairlog_append_playback(PairLog *pl, const unsigned char *buf, int offset, int size, GstClockTime time,
                                    int rate)
{
    gint64 i;
    ...
    if (!pl->playback) {
        ...
        GST_DEBUG("started playback log at %" GST_TIME_FORMAT, GST_TIME_ARGS(time));
    ...
    offset = (int)i;
    GST_LOG("start=%" GST_TIME_FORMAT ", time=%" GST_TIME_FORMAT ", offset=%d", GST_TIME_ARGS(pl->start),
...
static void pairlog_append_capture(PairLog *pl, const unsigned char *buf, int offset, int size, GstClockTime time,
                                   int rate)
{
    gint64 i;
    ...
    if (!pl->capture) {
        ...
        GST_DEBUG("started capture log at %" GST_TIME_FORMAT, GST_TIME_ARGS(time));
...
static gboolean have_env(const char *var)
{
...
static gboolean near_enough_to(GstClockTime a, GstClockTime b)
{
...
static void adapter_push_at(GstAdapter *adapter, GstBuffer *buffer, int offset)
{
    int        size;
    int        n, pad, start, len;
    int        end;
...
static void gst_speex_dsp_base_init(gpointer klass)
{
    GST_DEBUG_CATEGORY_INIT(speex_dsp_debug, "speexdsp", 0, "libspeexdsp wrapping elements");
}

static void gst_speex_dsp_class_init(GstSpeexDSPClass *klass)
{
    ...
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&gst_speex_dsp_rec_src_template));
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&gst_speex_dsp_rec_sink_template));
    ...
    g_object_class_install_property(
        gobject_class, PROP_PROBE,
        g_param_spec_object("probe", "A probe that gathers the buffers to do echo cancellation on",
                            "This is a link to the probe that gets buffers to cancel the echo"
                            " against",
                            GST_TYPE_SPEEX_ECHO_PROBE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_LATENCY_TUNE,
        g_param_spec_int("latency-tune", "Add/remove latency",
                         "Use this to tune the latency value, in milliseconds, in case it is"
                         " detected incorrectly",
                         G_MININT, G_MAXINT, DEFAULT_LATENCY_TUNE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_AGC,
                                    g_param_spec_boolean("agc", "Automatic Gain Control state",
                                                         "Enable or disable automatic Gain Control state", DEFAULT_AGC,
                                                         G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_AGC_INCREMENT,
        g_param_spec_int("agc-increment", "Maximal gain increase in dB/second", "Maximal gain increase in dB/second",
                         G_MININT, G_MAXINT, DEFAULT_AGC_INCREMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_AGC_DECREMENT,
        g_param_spec_int("agc-decrement", "Maximal gain increase in dB/second", "Maximal gain increase in dB/second",
                         G_MININT, G_MAXINT, DEFAULT_AGC_DECREMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_AGC_LEVEL,
                                    g_param_spec_float("agc-level", "Automatic Gain Control level",
                                                       "Automatic Gain Control level", -G_MAXFLOAT, G_MAXFLOAT,
                                                       DEFAULT_AGC_LEVEL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_AGC_MAX_GAIN,
                                    g_param_spec_int("agc-max-gain", "Maximal gain in dB", "Maximal gain in dB",
                                                     G_MININT, G_MAXINT, DEFAULT_AGC_MAX_GAIN,
                                                     G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_DENOISE,
                                    g_param_spec_boolean("denoise", "Denoiser state",
                                                         "Enable or disable denoiser state", DEFAULT_DENOISE,
                                                         G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_ECHO_SUPPRESS,
                                    g_param_spec_int("echo-suppress", "Maximum attenuation of the residual echo in dB",
                                                     "Maximum attenuation of the residual echo in dB (negative number)",
                                                     G_MININT, 0, DEFAULT_ECHO_SUPPRESS,
                                                     G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_ECHO_SUPPRESS_ACTIVE,
    ...
    g_object_class_install_property(gobject_class, PROP_NOISE_SUPPRESS,
                                    g_param_spec_int("noise-suppress", "Maximum attenuation of the noise in dB",
                                                     "Maximum attenuation of the noise in dB (negative number)",
                                                     G_MININT, 0, DEFAULT_NOISE_SUPPRESS,
                                                     G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void gst_speex_dsp_init(GstSpeexDSP *self, GstSpeexDSPClass *klass)
{
    ...
    gst_pad_set_getcaps_function(self->rec_srcpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_getcaps));
    gst_pad_set_event_function(self->rec_srcpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_rec_event));
    gst_pad_set_query_function(self->rec_srcpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_query));
    gst_pad_set_query_type_function(self->rec_srcpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_query_type));
    ...
    gst_object_unref(template);
    gst_pad_set_chain_function(self->rec_sinkpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_rec_chain));
    gst_pad_set_getcaps_function(self->rec_sinkpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_getcaps));
    gst_pad_set_setcaps_function(self->rec_sinkpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_setcaps));
    gst_pad_set_event_function(self->rec_sinkpad, GST_DEBUG_FUNCPTR(gst_speex_dsp_rec_event));
...
static void gst_speex_dsp_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    ...
    case PROP_AGC:
        ...
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_AGC, &self->agc);
        break;
    case PROP_AGC_INCREMENT:
        self->agc_increment = g_value_get_int(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_AGC_INCREMENT, &self->agc_increment);
        break;
    case PROP_AGC_DECREMENT:
        self->agc_decrement = g_value_get_int(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_AGC_DECREMENT, &self->agc_decrement);
        break;
    case PROP_AGC_LEVEL:
        self->agc_level = g_value_get_float(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_AGC_LEVEL, &self->agc_level);
        break;
    case PROP_AGC_MAX_GAIN:
        self->agc_max_gain = g_value_get_int(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_AGC_MAX_GAIN, &self->agc_max_gain);
        break;
    case PROP_DENOISE:
        self->denoise = g_value_get_boolean(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_DENOISE, &self->denoise);
        break;
    case PROP_ECHO_SUPPRESS:
        self->echo_suppress = g_value_get_int(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_ECHO_SUPPRESS, &self->echo_suppress);
        break;
    case PROP_ECHO_SUPPRESS_ACTIVE:
        self->echo_suppress_active = g_value_get_int(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_ECHO_SUPPRESS_ACTIVE,
                                 &self->echo_suppress_active);
        break;
    case PROP_NOISE_SUPPRESS:
        self->noise_suppress = g_value_get_int(value);
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_NOISE_SUPPRESS, &self->noise_suppress);
        break;
    default:
...
static void gst_speex_dsp_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    ...
    case PROP_AGC:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_AGC, &self->agc);
        g_value_set_boolean(value, self->agc);
        break;
    case PROP_AGC_INCREMENT:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_AGC_INCREMENT, &self->agc_increment);
        g_value_set_int(value, self->agc_increment);
        break;
    case PROP_AGC_DECREMENT:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_AGC_DECREMENT, &self->agc_decrement);
        g_value_set_int(value, self->agc_decrement);
        break;
    case PROP_AGC_LEVEL:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_AGC_LEVEL, &self->agc_level);
        g_value_set_float(value, self->agc_level);
        break;
    case PROP_AGC_MAX_GAIN:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_AGC_MAX_GAIN, &self->agc_max_gain);
        g_value_set_int(value, self->agc_max_gain);
        break;
    case PROP_DENOISE:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_DENOISE, &self->denoise);
        g_value_set_boolean(value, self->denoise);
        break;
    case PROP_ECHO_SUPPRESS:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_ECHO_SUPPRESS, &self->echo_suppress);
        g_value_set_int(value, self->echo_suppress);
        break;
    case PROP_ECHO_SUPPRESS_ACTIVE:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_ECHO_SUPPRESS_ACTIVE,
                                 &self->echo_suppress_active);
        g_value_set_int(value, self->echo_suppress_active);
        break;
    case PROP_NOISE_SUPPRESS:
        if (self->preprocstate)
            speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_GET_NOISE_SUPPRESS, &self->noise_suppress);
...
static GstCaps *gst_speex_dsp_getcaps(GstPad *pad)
{
    ...
    if (self->echostate != NULL) {
        GST_OBJECT_LOCK(self);
        gst_caps_set_simple(result, "rate", G_TYPE_INT, self->rate, "channels", G_TYPE_INT, self->channels, NULL);
        GST_OBJECT_UNLOCK(self);
        goto out;
    }
...
static gboolean gst_speex_dsp_setcaps(GstPad *pad, GstCaps *caps)
{
    ...
    GST_LOG_OBJECT(self, "setting caps on pad %p,%s to %" GST_PTR_FORMAT, pad, GST_PAD_NAME(pad), caps);
    ...
    if (self->echostate) {
        if (self->rate != rate) {
            GST_WARNING_OBJECT(self, "Wrong rate, got %d, expected %d", rate, self->rate);
            ret = FALSE;
        }
        if (self->channels != channels) {
            GST_WARNING_OBJECT(self, "Wrong channel count, got %d, expected %d", channels, self->channels);
            ret = FALSE;
        }
        goto done;
    }
    ...
    if (self->probe) {
        ...
        if (self->probe->rate) {
            if (self->probe->rate != rate) {
                GST_WARNING_OBJECT(self, "Wrong rate, probe has %d, we have %d", self->probe->rate, rate);
                ret = FALSE;
            } else {
    ...
    /*if (self->probe)*/ {
        ...
        if (self->channels == 1 && probe_channels == 1) {
            GST_DEBUG_OBJECT(self, "speex_echo_state_init (%d, %d)", frame_size, filter_length);
            self->echostate = speex_echo_state_init(frame_size, filter_length);
        } else {
            GST_DEBUG_OBJECT(self, "speex_echo_state_init_mc (%d, %d, %d, %d)", frame_size, filter_length,
                             self->channels, probe_channels);
            self->echostate = speex_echo_state_init_mc(frame_size, filter_length, self->channels, probe_channels);
        }
    }

    self->preprocstate = speex_preprocess_state_init(rate * self->frame_size_ms / 1000, rate);

    if (self->echostate) {
        speex_echo_ctl(self->echostate, SPEEX_ECHO_SET_SAMPLING_RATE, &rate);

        speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_ECHO_STATE, self->echostate);
    }
    ...
    speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_DENOISE, &self->denoise);
    speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_ECHO_SUPPRESS, &self->echo_suppress);
    speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_ECHO_SUPPRESS_ACTIVE, &self->echo_suppress_active);
    speex_preprocess_ctl(self->preprocstate, SPEEX_PREPROCESS_SET_NOISE_SUPPRESS, &self->noise_suppress);
...
static gboolean gst_speex_dsp_rec_event(GstPad *pad, GstEvent *event)
{
    ...
    case GST_EVENT_NEWSEGMENT: /* synchronized */
    {
        ...
        gst_segment_set_newsegment_full(&self->rec_segment, update, rate, applied_rate, format, start, stop, position);
    }
    default:
        break;
    }
    ...
    case GST_QUERY_LATENCY: {
        ...
        if ((peer = gst_pad_get_peer(self->rec_sinkpad))) {
            if ((res = gst_pad_query(peer, query))) {
                ...
                GST_DEBUG_OBJECT(self, "Peer latency: min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT,
                                 GST_TIME_ARGS(min), GST_TIME_ARGS(max));

                /* add our own latency */
                latency = ((guint64)self->frame_size_ms) * 1000000; /* to nanos */

                GST_DEBUG_OBJECT(self, "Our latency: %" GST_TIME_FORMAT, GST_TIME_ARGS(latency));

                min += latency;
                if (max != GST_CLOCK_TIME_NONE)
                    max += latency;

                GST_DEBUG_OBJECT(self, "Calculated total latency : min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT,
                                 GST_TIME_ARGS(min), GST_TIME_ARGS(max));
...
static GstFlowReturn gst_speex_dsp_rec_chain(GstPad *pad, GstBuffer *buffer)
{
    ...
    if (gst_adapter_available(self->rec_adapter) == 0) {
        GST_LOG_OBJECT(self,
                       "The adapter is empty, its a good time to reset the"
    ...
    if (self->rec_time != GST_CLOCK_TIME_NONE) {
        ...
        if (!near_enough_to(a, b)) {
            GST_LOG_OBJECT(self,
                           "***continuous buffer with wrong timestamp"
    ...
    {
        GstClockTime rec_rt;
        rec_rt = gst_segment_to_running_time(&self->rec_segment, GST_FORMAT_TIME, GST_BUFFER_TIMESTAMP(buffer));

        GST_LOG_OBJECT(
            self, "Captured buffer at %" GST_TIME_FORMAT " (len=%" GST_TIME_FORMAT ", offset=%lld, base=%lld)",
            // GST_TIME_ARGS (GST_BUFFER_TIMESTAMP (buffer) /*+ base_time*/),
            GST_TIME_ARGS(rec_rt), GST_TIME_ARGS(GST_BUFFER_DURATION(buffer)), GST_BUFFER_OFFSET(buffer), base_time);
    }

    g_static_mutex_lock(&pairlog_mutex);
    if (pairlog) {
        pairlog_append_capture(pairlog, (const unsigned char *)GST_BUFFER_DATA(buffer), GST_BUFFER_OFFSET(buffer) * 2,
                               GST_BUFFER_SIZE(buffer), GST_BUFFER_TIMESTAMP(buffer) + skew_fix, rate);
    }
    g_static_mutex_unlock(&pairlog_mutex);
    ...
    while (TRUE) {
        ...
        outbuffer = try_echo_cancel(self->echostate, recbuffer,
                                    ...
                                    base_time, self->buffers, rate, self->rec_srcpad, GST_PAD_CAPS(self->rec_sinkpad),
                                    &res, self);
        GST_OBJECT_UNLOCK(self);
        if (outbuffer) {
        ...
        GST_OBJECT_LOCK(self);
        speex_preprocess_run(self->preprocstate, (spx_int16_t *)GST_BUFFER_DATA(outbuffer));
        GST_OBJECT_UNLOCK(self);
...
void gst_speex_dsp_add_capture_buffer(GstSpeexDSP *self, GstBuffer *buf)
{
    ...
    GST_LOG_OBJECT(self, "Played buffer at %" GST_TIME_FORMAT " (len=%" GST_TIME_FORMAT ", offset=%lld)",
                   GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf) - base_time), GST_TIME_ARGS(duration),
                   GST_BUFFER_OFFSET(buf));

    g_static_mutex_lock(&pairlog_mutex);
    if (pairlog && rate != 0) {
        pairlog_append_playback(pairlog, (const unsigned char *)GST_BUFFER_DATA(buf), GST_BUFFER_OFFSET(buf) * 2,
                                GST_BUFFER_SIZE(buf), GST_BUFFER_TIMESTAMP(buf) - base_time, rate);
    }
    g_static_mutex_unlock(&pairlog_mutex);
 ...
 */
GstBuffer *try_echo_cancel(SpeexEchoState *echostate, const GstBuffer *recbuf, GstClockTime rec_adj,
                           GstClockTime rec_base, GQueue *buffers, int rate, GstPad *srcpad, const GstCaps *outcaps,
                           GstFlowReturn *res, GstSpeexDSP *obj)
{
    ...
    while (TRUE) {
        ...
        if (!play_buffer
            || GST_BUFFER_TIMESTAMP(play_buffer) - rec_base + (GST_BUFFER_SIZE(play_buffer) * GST_SECOND / (rate * 2))
                >= rec_rt)
            break;
    ...
    res_ = gst_pad_alloc_buffer(srcpad, rec_offset, bufsize, (GstCaps *)outcaps, &outbuffer);
    if (res_ != GST_FLOW_OK) {
        *res = res_;
        return NULL;
    }
    ...
    while (rec_offset < bufsize) {
        ...
        if (rec_end < play_rt) {
            GST_LOG_OBJECT(obj,
                           "End of recorded buffer (at %" GST_TIME_FORMAT ")"
                           " is before any played buffer"
                           " (which start at %" GST_TIME_FORMAT ")",
                           GST_TIME_ARGS(rec_end), GST_TIME_ARGS(play_rt));
            break;
        }

        play_duration = GST_BUFFER_SIZE(play_buffer) * GST_SECOND / (rate * 2);
        ...
        if (play_rt + play_duration < rec_rt) {
            GST_LOG_OBJECT(obj,
                           "Start of rec data (at %" GST_TIME_FORMAT ")"
                           " after the end of played data (at %" GST_TIME_FORMAT ")",
                           GST_TIME_ARGS(rec_rt), GST_TIME_ARGS(play_rt + play_duration));
            g_queue_pop_tail(buffers);
            gst_buffer_unref(play_buffer);
            continue;
        }
        ...
        size        = MIN(GST_BUFFER_SIZE(play_buffer) - play_offset, bufsize - rec_offset);
        ...
        GST_LOG_OBJECT(obj,
                       "Cancelling data recorded at %" GST_TIME_FORMAT " with data played at %" GST_TIME_FORMAT
                       " (difference %" GST_TIME_FORMAT ") for %d bytes",
                       GST_TIME_ARGS(rec_rt), GST_TIME_ARGS(time), GST_TIME_ARGS(rec_rt - time), size);

        GST_LOG_OBJECT(obj, "using play buffer %p (size=%d), mid(%d, %d)", play_buffer, GST_BUFFER_SIZE(play_buffer),
                       play_offset, size);

        if (rec_offset < 0 || play_offset < 0 || rec_offset + size > bufsize
            || play_offset + size > GST_BUFFER_SIZE(play_buffer)) {
            fprintf(stderr, "***speexdsp explosions!\n");
            abort();
            return NULL;
        }

        memcpy(buf + rec_offset, GST_BUFFER_DATA(play_buffer) + play_offset, size);
    ...
    speex_echo_cancellation(echostate, (const spx_int16_t *)GST_BUFFER_DATA(recbuffer), (const spx_int16_t *)buf,
