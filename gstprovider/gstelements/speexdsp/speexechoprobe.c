...
static const GstElementDetails gst_speex_echo_probe_details
    = GST_ELEMENT_DETAILS("Accoustic Echo canceller probe", "Generic/Audio", "Gathers playback buffers for speexdsp",
...
enum { PROP_0, PROP_LATENCY_TUNE };

GST_BOILERPLATE(GstSpeexEchoProbe, gst_speex_echo_probe, GstElement, GST_TYPE_ELEMENT);

static void gst_speex_echo_probe_finalize(GObject *object);
static void gst_speex_echo_probe_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gst_speex_echo_probe_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static GstStateChangeReturn gst_speex_echo_probe_change_state(GstElement *element, GstStateChange transition);

static GstFlowReturn gst_speex_echo_probe_chain(GstPad *pad, GstBuffer *buffer);
static gboolean      gst_speex_echo_probe_event(GstPad *pad, GstEvent *event);
static gboolean      gst_speex_echo_probe_setcaps(GstPad *pad, GstCaps *caps);
static GstCaps *     gst_speex_echo_probe_getcaps(GstPad *pad);

static void try_auto_attach();

static void gst_speex_echo_probe_base_init(gpointer klass) {}

static void gst_speex_echo_probe_class_init(GstSpeexEchoProbeClass *klass)
{
    ...
    gst_element_class_set_details(gstelement_class, &gst_speex_echo_probe_details);
...
static void gst_speex_echo_probe_init(GstSpeexEchoProbe *self, GstSpeexEchoProbeClass *klass)
{
    ...
    gst_pad_set_event_function(self->srcpad, GST_DEBUG_FUNCPTR(gst_speex_echo_probe_event));
    gst_pad_set_getcaps_function(self->srcpad, GST_DEBUG_FUNCPTR(gst_speex_echo_probe_getcaps));
    ...
    gst_object_unref(template);
    gst_pad_set_chain_function(self->sinkpad, GST_DEBUG_FUNCPTR(gst_speex_echo_probe_chain));
    gst_pad_set_event_function(self->sinkpad, GST_DEBUG_FUNCPTR(gst_speex_echo_probe_event));
    gst_pad_set_setcaps_function(self->sinkpad, GST_DEBUG_FUNCPTR(gst_speex_echo_probe_setcaps));
    gst_pad_set_getcaps_function(self->sinkpad, GST_DEBUG_FUNCPTR(gst_speex_echo_probe_getcaps));
...
static gboolean gst_speex_echo_probe_setcaps(GstPad *pad, GstCaps *caps)
{
    ...
    GST_DEBUG_OBJECT(self, "setting caps on pad %p,%s to %" GST_PTR_FORMAT, pad, GST_PAD_NAME(pad), caps);
    ...
    if (self->rate && self->rate != rate) {
        ...
        GST_WARNING_OBJECT(self, "Wrong channels, got %d, expected %d", channels, self->channels);
        ret = FALSE;
    }
...
static gboolean gst_speex_echo_probe_event(GstPad *pad, GstEvent *event)
{
    ...
    case GST_EVENT_LATENCY:
        ...
        GST_DEBUG_OBJECT(self, "We have a latency of %" GST_TIME_FORMAT, GST_TIME_ARGS(latency));
        break;
    ...
    case GST_EVENT_NEWSEGMENT: {
        ...
        GST_OBJECT_LOCK(self);
        gst_segment_set_newsegment_full(&self->segment, update, rate, applied_rate, format, start, stop, position);
        GST_OBJECT_UNLOCK(self);
    } break;
    default:
        break;
