...
static const GstElementDetails gst_live_adder_details
    = GST_ELEMENT_DETAILS("Live Adder element", "Generic/Audio", "Mixes live/discontinuous audio streams",
                          "Olivier Crete <olivier.crete@collabora.co.uk>");

static GstStaticPadTemplate gst_live_adder_sink_template
    = GST_STATIC_PAD_TEMPLATE("sink%d", GST_PAD_SINK, GST_PAD_REQUEST,
...
static void gst_live_adder_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gst_live_adder_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static GstPad *gst_live_adder_request_new_pad(GstElement *element, GstPadTemplate *templ, const gchar *unused);
static void    gst_live_adder_release_pad(GstElement *element, GstPad *pad);
static GstStateChangeReturn gst_live_adder_change_state(GstElement *element, GstStateChange transition);

static gboolean gst_live_adder_setcaps(GstPad *pad, GstCaps *caps);
static GstCaps *gst_live_adder_sink_getcaps(GstPad *pad);
static gboolean gst_live_adder_src_activate_push(GstPad *pad, gboolean active);
...
static void gst_live_adder_base_init(gpointer klass) {}

static void gst_live_adder_class_init(GstLiveAdderClass *klass)
{
    ...
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&gst_live_adder_src_template));
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&gst_live_adder_sink_template));
    ...
    g_object_class_install_property(gobject_class, PROP_LATENCY,
                                    g_param_spec_uint("latency", "Buffer latency in ms", "Amount of data to buffer", 0,
                                                      G_MAXUINT, DEFAULT_LATENCY_MS, G_PARAM_READWRITE));

    GST_DEBUG_CATEGORY_INIT(live_adder_debug, "liveadder", 0, "Live Adder");
}

static void gst_live_adder_init(GstLiveAdder *adder, GstLiveAdderClass *klass)
{
    ...
    gst_pad_set_setcaps_function(adder->srcpad, GST_DEBUG_FUNCPTR(gst_live_adder_setcaps));
    gst_pad_set_query_function(adder->srcpad, GST_DEBUG_FUNCPTR(gst_live_adder_query));
    gst_pad_set_event_function(adder->srcpad, GST_DEBUG_FUNCPTR(gst_live_adder_src_event));
    gst_pad_set_activatepush_function(adder->srcpad, GST_DEBUG_FUNCPTR(gst_live_adder_src_activate_push));
...
static void gst_live_adder_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    ...
    case PROP_LATENCY: {
        ...
        if (new_latency != old_latency) {
            GST_DEBUG_OBJECT(adder, "latency changed to: %" GST_TIME_FORMAT, GST_TIME_ARGS(new_latency));

            gst_element_post_message(GST_ELEMENT_CAST(adder), gst_message_new_latency(GST_OBJECT_CAST(adder)));
        }
        break;
    }
    default:
...
static gboolean gst_live_adder_setcaps(GstPad *pad, GstCaps *caps)
{
    ...
    if (strcmp(media_type, "audio/x-raw-int") == 0) {
        ...
        switch (adder->width) {
        case 8:
            adder->func = (adder->is_signed ? (GstLiveAdderFunction)add_int8 : (GstLiveAdderFunction)add_uint8);
            break;
        case 16:
            adder->func = (adder->is_signed ? (GstLiveAdderFunction)add_int16 : (GstLiveAdderFunction)add_uint16);
            break;
        case 32:
            adder->func = (adder->is_signed ? (GstLiveAdderFunction)add_int32 : (GstLiveAdderFunction)add_uint32);
            break;
        default:
            goto not_supported;
        }
...
static void gst_live_adder_flush_start(GstLiveAdder *adder)
{
...
static gboolean gst_live_adder_src_activate_push(GstPad *pad, gboolean active)
{
    ...
    if (active) {
        ...
        gst_pad_start_task(adder->srcpad, (GstTaskFunction)gst_live_adder_loop, adder);
    } else {
...
static gboolean gst_live_adder_sink_event(GstPad *pad, GstEvent *event)
{
    ...
    case GST_EVENT_NEWSEGMENT: {
        ...
        GST_DEBUG_OBJECT(adder,
                         "newsegment: update %d, rate %g, arate %g, start %" GST_TIME_FORMAT ", stop %" GST_TIME_FORMAT
                         ", time %" GST_TIME_FORMAT,
                         update, rate, arate, GST_TIME_ARGS(start), GST_TIME_ARGS(stop), GST_TIME_ARGS(time));

        /* now configure the values, we need these to time the release of the
         * buffers on the srcpad. */
        GST_OBJECT_LOCK(adder);
        gst_segment_set_newsegment_full(&padprivate->segment, update, rate, arate, format, start, stop, time);
        GST_OBJECT_UNLOCK(adder);
        break;
    }
    ...
    case GST_EVENT_EOS: {
        ...
        if (ret && !padprivate->eos) {
            ...
            GST_DEBUG_OBJECT(adder, "dropping EOS, reason %s", gst_flow_get_name(adder->srcresult));
        }
...
static gboolean gst_live_adder_query_pos_dur(GstLiveAdder *adder, GstFormat informat, gboolean position,
                                             gint64 *outvalue)
{
 ...
 */
static gboolean gst_live_adder_query_duration(GstLiveAdder *adder, GstQuery *query)
{
    GstFormat format;
    gint64    max;
    gboolean  res;
...
static gboolean gst_live_adder_query_position(GstLiveAdder *adder, GstQuery *query)
{
    GstFormat format;
    gint64    max;
    gboolean  res;
...
static gboolean gst_live_adder_query(GstPad *pad, GstQuery *query)
{
    ...
    case GST_QUERY_LATENCY: {
        ...
        while (!done) {
            ...
            case GST_ITERATOR_OK: {
                ...
                if (gst_pad_peer_query(sinkpad, query)) {
                    gst_query_parse_latency(query, &pad_us_live, &pad_min_latency, &pad_max_latency);

                    res = TRUE;

                    GST_DEBUG_OBJECT(adder, "Peer latency for pad %s: min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT,
                                     GST_PAD_NAME(sinkpad), GST_TIME_ARGS(pad_min_latency),
        ...
        if (res) {
            ...
            GST_DEBUG_OBJECT(adder, "Calculated total latency : min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT,
 ...
 * sinkpads.
 */
static gboolean forward_event(GstLiveAdder *adder, GstEvent *event)
{
    gboolean     ret;
    GstIterator *it;
    GValue       vret = { 0 };
...
static guint gst_live_adder_length_from_duration(GstLiveAdder *adder, GstClockTime duration)
{
...
static GstFlowReturn gst_live_live_adder_chain(GstPad *pad, GstBuffer *buffer)
{
    ...
    if (ret != GST_FLOW_OK) {
        GST_DEBUG_OBJECT(adder, "Passing non-ok result from src: %s", gst_flow_get_name(ret));
    ...
    if (padprivate->segment.format == GST_FORMAT_UNDEFINED) {
        GST_WARNING_OBJECT(adder,
                           "No new-segment received,"
                           " initializing segment with time 0..-1");
        gst_segment_init(&padprivate->segment, GST_FORMAT_TIME);
        gst_segment_set_newsegment(&padprivate->segment, FALSE, 1.0, GST_FORMAT_TIME, 0, -1, 0);
    }
    ...
    if (GST_CLOCK_TIME_IS_VALID(padprivate->expected_timestamp)
        && !GST_BUFFER_FLAG_IS_SET(buffer, GST_BUFFER_FLAG_DISCONT) && (drift != 0)) {
        GST_LOG_OBJECT(adder,
                       "Timestamp discontinuity without the DISCONT flag set"
                       " (expected %" GST_TIME_FORMAT ", got %" GST_TIME_FORMAT " drift:%ldms)",
                       GST_TIME_ARGS(padprivate->expected_timestamp), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer)),
    ...
    if (!GST_BUFFER_DURATION_IS_VALID(buffer)) {
        GST_BUFFER_DURATION(buffer)    = gst_audio_duration_from_pad_buffer(pad, buffer);
        padprivate->expected_timestamp = GST_CLOCK_TIME_NONE;
    } else {
        padprivate->expected_timestamp = GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer);
    }
    ...
    buffer = gst_audio_buffer_clip(buffer, &padprivate->segment, adder->rate, adder->bps);
    ...
    if (GST_CLOCK_TIME_IS_VALID(adder->next_timestamp) && GST_BUFFER_TIMESTAMP(buffer) < adder->next_timestamp) {
        if (GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer) < adder->next_timestamp) {
            GST_DEBUG_OBJECT(adder,
                             "Buffer is late, dropping (ts: %" GST_TIME_FORMAT " duration: %" GST_TIME_FORMAT ")",
                             GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer)), GST_TIME_ARGS(GST_BUFFER_DURATION(buffer)));
            gst_buffer_unref(buffer);
            goto out;
        } else {
            skip = adder->next_timestamp - GST_BUFFER_TIMESTAMP(buffer);
            GST_DEBUG_OBJECT(adder, "Buffer is partially late, skipping %" GST_TIME_FORMAT, GST_TIME_ARGS(skip));
        }
    }
     ...
     */
    if (adder->clock_id && g_queue_peek_head(adder->buffers) != NULL
        && GST_BUFFER_TIMESTAMP(buffer) + skip < GST_BUFFER_TIMESTAMP(g_queue_peek_head(adder->buffers)))
        gst_clock_id_unschedule(adder->clock_id);

    for (item = g_queue_peek_head_link(adder->buffers); item; item = g_list_next(item)) {
        ...
        if (GST_BUFFER_TIMESTAMP(buffer) + skip >= GST_BUFFER_TIMESTAMP(oldbuffer) + GST_BUFFER_DURATION(oldbuffer))
            continue;

        /* We're past our place, lets insert ouselves here */
        if (GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer) <= GST_BUFFER_TIMESTAMP(oldbuffer))
            break;
        ...
        if (GST_BUFFER_TIMESTAMP(buffer) + skip < GST_BUFFER_TIMESTAMP(oldbuffer)) {
            ...
            GstClockTime subbuffer_duration = GST_BUFFER_TIMESTAMP(oldbuffer) - (GST_BUFFER_TIMESTAMP(buffer) + skip);

            subbuffer = gst_buffer_create_sub(buffer, gst_live_adder_length_from_duration(adder, skip),
        ...
        old_skip = GST_BUFFER_TIMESTAMP(buffer) + skip - GST_BUFFER_TIMESTAMP(oldbuffer);
        ...
        if (GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer)
            ...
            mix_end = GST_BUFFER_TIMESTAMP(oldbuffer) + GST_BUFFER_DURATION(oldbuffer);

        mix_duration = mix_end - mix_start;

        adder->func(GST_BUFFER_DATA(oldbuffer) + gst_live_adder_length_from_duration(adder, old_skip),
                    GST_BUFFER_DATA(buffer) + gst_live_adder_length_from_duration(adder, skip),
    ...
    if (skip == GST_BUFFER_DURATION(buffer)) {
        ...
        if (skip) {
            ...
            buffer = gst_buffer_create_sub(buffer, gst_live_adder_length_from_duration(adder, skip),
                                           gst_live_adder_length_from_duration(adder, subbuffer_duration));
...
invalid_timestamp:
    ...
    GST_OBJECT_UNLOCK(adder);
    gst_buffer_unref(buffer);
    GST_ELEMENT_ERROR(adder, STREAM, FAILED, ("Buffer without a valid timestamp received"),
...
invalid_segment : {
    ...
    GST_OBJECT_UNLOCK(adder);
    gst_buffer_unref(buffer);
    GST_ELEMENT_ERROR(adder, STREAM, FAILED, ("This element only supports TIME segments, received other type"),
 ...
 */
static gboolean check_eos_locked(GstLiveAdder *adder)
{
    GList *item;
...
again:
    ...
    GST_DEBUG_OBJECT(adder, "sync to timestamp %" GST_TIME_FORMAT, GST_TIME_ARGS(buffer_timestamp));
    ...
    if (ret == GST_CLOCK_UNSCHEDULED) {
        GST_DEBUG_OBJECT(adder, "Wait got unscheduled, will retry to push with new buffer");
        goto again;
    }
...
push_buffer:
     ...
     */
    if (GST_CLOCK_TIME_IS_VALID(adder->next_timestamp) && GST_BUFFER_TIMESTAMP(buffer) != adder->next_timestamp) {
        GstClockTimeDiff diff = GST_CLOCK_DIFF(GST_BUFFER_TIMESTAMP(buffer), adder->next_timestamp);
        if (diff < 0)
            diff = -diff;
        ...
        if (diff < GST_SECOND / adder->rate) {
            ...
            GST_DEBUG_OBJECT(adder,
                             "Expected buffer at %" GST_TIME_FORMAT ", but is at %" GST_TIME_FORMAT ", setting discont",
                             GST_TIME_ARGS(adder->next_timestamp), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer)));
        }
    } else {
        GST_BUFFER_FLAG_UNSET(buffer, GST_BUFFER_FLAG_DISCONT);
    }
    ...
    if (GST_BUFFER_DURATION_IS_VALID(buffer))
        adder->next_timestamp = GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer);
    else
        adder->next_timestamp = GST_CLOCK_TIME_NONE;
    ...
    if (adder->segment_pending) {
         ...
         */
        newseg_event = gst_event_new_new_segment_full(FALSE, 1.0, 1.0, GST_FORMAT_TIME, 0, -1, 0);
...
clock_error : {
    gst_pad_pause_task(adder->srcpad);
    GST_OBJECT_UNLOCK(adder);
    GST_ELEMENT_ERROR(adder, STREAM, MUX, ("Error with the clock"), ("Error with the clock: %d", ret));
...
no_clock : {
    gst_pad_pause_task(adder->srcpad);
    GST_OBJECT_UNLOCK(adder);
    GST_ELEMENT_ERROR(adder, STREAM, MUX, ("No available clock"), ("No available clock"));
...
static GstPad *gst_live_adder_request_new_pad(GstElement *element, GstPadTemplate *templ, const gchar *unused)
{
    ...
    gst_pad_set_getcaps_function(newpad, GST_DEBUG_FUNCPTR(gst_live_adder_sink_getcaps));
    gst_pad_set_setcaps_function(newpad, GST_DEBUG_FUNCPTR(gst_live_adder_setcaps));
    gst_pad_set_event_function(newpad, GST_DEBUG_FUNCPTR(gst_live_adder_sink_event));
...
{
    if (!gst_element_register(plugin, "liveadder", GST_RANK_NONE, GST_TYPE_LIVE_ADDER)) {
        return FALSE;
    }
...
{
    gst_plugin_register_static(GST_VERSION_MAJOR, GST_VERSION_MINOR, "liveadder",
                               "Adds multiple live discontinuous streams", plugin_init, "1.0.4", "LGPL",
