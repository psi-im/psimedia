...
static gchar *guid_to_string(LPGUID in)
{
    WCHAR buffer[256];
...
static LPGUID string_to_guid(const gchar *str)
{
    HRESULT    ret;
    gunichar2 *wstr;
    LPGUID     out;
...
static BOOL CALLBACK cb_enum(LPGUID lpGUID, LPCWSTR lpszDesc, LPCWSTR lpszDrvName, LPVOID lpContext)
{
    GList **                list;
    ...
    dev->name = g_utf16_to_utf8((const gunichar2 *)lpszDesc, -1, NULL, NULL, NULL);
    if (!dev->name) {
        gst_directsound_device_free(dev);
        return TRUE;
    }
...
LPGUID
gst_directsound_get_device_guid(const gchar *id) { return string_to_guid(id); }
