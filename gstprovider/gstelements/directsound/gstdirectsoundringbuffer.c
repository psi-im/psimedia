...
GST_BOILERPLATE(GstDirectSoundRingBuffer, gst_directsound_ring_buffer, GstRingBuffer, GST_TYPE_RING_BUFFER);

static void gst_directsound_ring_buffer_base_init(gpointer g_class) { /* Nothing to do right now */ }
...
static gboolean gst_directsound_ring_buffer_open_device(GstRingBuffer *buf)
{
    ...
    if (dsoundbuffer->is_src) {
        ...
        if (FAILED(hr = DirectSoundCaptureCreate8(lpGuid, &dsoundbuffer->pDSC8, NULL))) {
            GST_ELEMENT_ERROR(dsoundbuffer->element, RESOURCE, FAILED, ("%ls.", DXGetErrorDescription9W(hr)),
...
static gboolean gst_directsound_create_buffer(GstRingBuffer *buf)
{
    ...
    if (dsoundbuffer->is_src) {
        ...
        hr = IDirectSoundCapture_CreateCaptureBuffer(dsoundbuffer->pDSC8, &captureDescSecondary, &pDSCB, NULL);
        if (G_UNLIKELY(FAILED(hr))) {
            GST_WARNING("gst_directsound_ring_buffer_acquire: IDirectSoundCapture_CreateCaptureBuffer, hr = %X",
                        (unsigned int)hr);
            return FALSE;
        }

        hr = IDirectSoundCaptureBuffer_QueryInterface(pDSCB, &IID_IDirectSoundCaptureBuffer8,
                                                      (LPVOID *)&dsoundbuffer->pDSCB8);
        if (G_UNLIKELY(FAILED(hr))) {
            IDirectSoundCaptureBuffer_Release(pDSCB);
            GST_WARNING("gst_directsound_ring_buffer_acquire: IDirectSoundCaptureBuffer_QueryInterface, hr = %X",
                        (unsigned int)hr);
            return FALSE;
        }
        ...
        if (G_UNLIKELY(FAILED(hr))) {
            GST_WARNING("gst_directsound_ring_buffer_acquire: IDirectSound8_CreateSoundBuffer, hr = %X",
                        (unsigned int)hr);
            return FALSE;
        }
...
static gboolean gst_directsound_ring_buffer_start(GstRingBuffer *buf)
{
    ...
    if (dsoundbuffer->is_src) {
        hThread = CreateThread(NULL, 256 * 1024 /* Stack size: 256k */, gst_directsound_read_proc, buf,
                               CREATE_SUSPENDED, NULL);
    } else {
        hThread = CreateThread(NULL, 256 * 1024 /* Stack size: 256k */, gst_directsound_write_proc, buf,
                               CREATE_SUSPENDED, NULL);
    }
...
static gboolean gst_directsound_ring_buffer_stop(GstRingBuffer *buf)
{
    ...
    if (dsoundbuffer->is_src) {
        if (dsoundbuffer->pDSCB8) {
            ...
            if (G_UNLIKELY(FAILED(hr))) {
                GST_DSOUND_UNLOCK(dsoundbuffer);
                GST_WARNING("gst_directsound_ring_buffer_stop: IDirectSoundCaptureBuffer_Stop, hr = %X",
                            (unsigned int)hr);
                return FALSE;
            }
        }
...
static guint gst_directsound_ring_buffer_delay(GstRingBuffer *buf)
{
    ...
    if (dsoundbuffer->is_src) {
        if (G_LIKELY(dsoundbuffer->pDSCB8)) {
            ...
            hr = IDirectSoundCaptureBuffer8_GetCurrentPosition(dsoundbuffer->pDSCB8, &dwCurrentCaptureCursor,
                                                               &dwCurrentReadCursor);

            if (G_LIKELY(SUCCEEDED(hr))) {
                if (dwCurrentCaptureCursor >= dsoundbuffer->buffer_circular_offset)
                    dwBytesInQueue = dwCurrentCaptureCursor - dsoundbuffer->buffer_circular_offset;
                else
                    dwBytesInQueue
                        = dwCurrentCaptureCursor + (dsoundbuffer->buffer_size - dsoundbuffer->buffer_circular_offset);

                nNbSamplesInQueue = dwBytesInQueue / dsoundbuffer->bytes_per_sample;
            } else {
                GST_WARNING("gst_directsound_ring_buffer_delay: IDirectSoundCaptureBuffer8_GetCurrentPosition, hr = %X",
                            (unsigned int)hr);
            }
        }
    } else {
        if (G_LIKELY(dsoundbuffer->pDSB8)) {
            /* evaluate the number of samples in queue in the circular buffer */
            hr = IDirectSoundBuffer8_GetCurrentPosition(dsoundbuffer->pDSB8, &dwCurrentPlayCursor,
                                                        &dwCurrentWriteCursor);

            if (G_LIKELY(SUCCEEDED(hr))) {
                if (dwCurrentPlayCursor <= dsoundbuffer->buffer_circular_offset)
                    dwBytesInQueue = dsoundbuffer->buffer_circular_offset - dwCurrentPlayCursor;
                else
                    dwBytesInQueue
                        = dsoundbuffer->buffer_circular_offset + (dsoundbuffer->buffer_size - dwCurrentPlayCursor);
...
static DWORD WINAPI gst_directsound_write_proc(LPVOID lpParameter)
{
    ...
    restore_buffer:
        ...
        if (dwStatus & DSBSTATUS_BUFFERLOST) {
            ...
            if (FAILED(hr) && hr == DSERR_BUFFERLOST) {
                if (retries++ < MAX_LOST_RETRIES) {
                    ...
                    GST_ELEMENT_ERROR(
                        dsoundbuffer->element, RESOURCE, FAILED, ("%ls.", DXGetErrorDescription9W(hr)),
        ...
        GST_DSOUND_LOCK(dsoundbuffer);
        if (dwCurrentPlayCursor <= dsoundbuffer->buffer_circular_offset)
            freeBufferSize = dsoundbuffer->buffer_size - (dsoundbuffer->buffer_circular_offset - dwCurrentPlayCursor);
        else
        ...
        /* lock it */
        GST_DSOUND_LOCK(dsoundbuffer);
        hr = IDirectSoundBuffer8_Lock(dsoundbuffer->pDSB8, dsoundbuffer->buffer_circular_offset, len, &pLockedBuffer1,
    ...
    if (dwStatus & DSBSTATUS_BUFFERLOST) {
      ...
      if (FAILED(hr) && hr == DSERR_BUFFERLOST) {
        ...
        if (!(dwStatus & DSCBSTATUS_CAPTURING)) {
            hr = IDirectSoundCaptureBuffer8_Start(dsoundbuffer->pDSCB8, DSCBSTART_LOOPING);
            if (FAILED(hr)) {
        ...
        if (G_UNLIKELY(FAILED(hr))) {
            ...
            if (FAILED(hr) && !error) {
                GST_ELEMENT_ERROR(dsoundbuffer->element, RESOURCE, FAILED, ("%ls.", DXGetErrorDescription9W(hr)),
                                  ("gst_directsound_read_proc: IDirectSoundCaptureBuffer8_GetCurrentPosition, hr = %X",
                                   (unsigned int)hr));
                error = TRUE;
                goto complete;
            }
        ...
        GST_LOG("Current Read Start: %u Current Read End: %d", dsoundbuffer->buffer_circular_offset,
        ...
        if (dwCurrentReadCursor >= dsoundbuffer->buffer_circular_offset)
            capturedBufferSize = dwCurrentReadCursor - dsoundbuffer->buffer_circular_offset;
        else
            capturedBufferSize
                = dsoundbuffer->buffer_size - (dsoundbuffer->buffer_circular_offset - dwCurrentReadCursor);
        GST_DSOUND_UNLOCK(dsoundbuffer);
        ...
        GST_LOG("Size of segment to read: %d Captured buffer size: %lld", len, capturedBufferSize);
        ...
        /* lock it */
        GST_DSOUND_LOCK(dsoundbuffer);
        hr = IDirectSoundCaptureBuffer8_Lock(dsoundbuffer->pDSCB8, dsoundbuffer->buffer_circular_offset, len,
        ...
        if (SUCCEEDED(hr)) {
            ...
            IDirectSoundCaptureBuffer8_Unlock(dsoundbuffer->pDSCB8, pLockedBuffer1, dwSizeBuffer1, pLockedBuffer2,
                                              dwSizeBuffer2);
        } else {
