...
#define GST_DEBUG_IS_ENABLED() (gst_debug_category_get_threshold(GST_CAT_DEFAULT) >= GST_LEVEL_DEBUG)
...
#define GST_KS_VIDEO_DEVICE_GET_PRIVATE(o)                                                                             \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), GST_TYPE_KS_VIDEO_DEVICE, GstKsVideoDevicePrivate))

static void gst_ks_video_device_dispose(GObject *object);
static void gst_ks_video_device_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
static void gst_ks_video_device_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
...
static void gst_ks_video_device_base_init(gpointer gclass) {}

static void gst_ks_video_device_class_init(GstKsVideoDeviceClass *klass)
{
    ...
    g_object_class_install_property(
        gobject_class, PROP_CLOCK,
        g_param_spec_object("clock", "Clock to use", "Clock to use", GST_TYPE_KS_CLOCK,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(
        gobject_class, PROP_DEVICE_PATH,
        g_param_spec_string("device-path", "Device Path", "The device path", DEFAULT_DEVICE_PATH,
                            G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}

static void gst_ks_video_device_init(GstKsVideoDevice *self, GstKsVideoDeviceClass *gclass)
{
...
static void gst_ks_video_device_parse_win32_error(const gchar *func_name, DWORD error_code, gulong *ret_error_code,
                                                  gchar **ret_error_str)
{
    ...
    if (ret_error_str != NULL) {
        ...
        result = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, error_code, 0, buf,
                               sizeof(buf), NULL);
        if (result != 0) {
            g_string_append_printf(message, "0x%08x: %s", error_code, g_strchomp(buf));
        } else {
            DWORD format_error_code = GetLastError();

            g_string_append_printf(message, "<0x%08x (FormatMessage error code: %s)>", error_code,
...
static void gst_ks_video_device_clear_buffers(GstKsVideoDevice *self)
{
    GstKsVideoDevicePrivate *priv = GST_KS_VIDEO_DEVICE_GET_PRIVATE(self);
    guint                    i;
    ...
    for (i = 0; i < priv->num_requests; i++) {
        ...
        GetOverlappedResult(priv->pin_handle, &req->overlapped, &bytes_returned, TRUE);
    }
...
static void gst_ks_video_device_prepare_buffers(GstKsVideoDevice *self)
{
    GstKsVideoDevicePrivate *priv = GST_KS_VIDEO_DEVICE_GET_PRIVATE(self);
    guint                    i;
    guint                    frame_size;
    ...
    priv->requests       = g_array_sized_new(FALSE, TRUE, sizeof(ReadRequest), priv->num_requests);
    priv->request_events = g_array_sized_new(FALSE, TRUE, sizeof(HANDLE), priv->num_requests + 1);
    ...
    for (i = 0; i < priv->num_requests; i++) {
        ...
        req.buf           = (guint8 *)(((gsize)req.buf_unaligned + BUFFER_ALIGNMENT - 1) & ~(BUFFER_ALIGNMENT - 1));
...
static void gst_ks_video_device_dump_supported_property_sets(GstKsVideoDevice *self, const gchar *obj_name,
                                                             const GUID *propsets, gulong propsets_len)
{
    guint i;

    GST_DEBUG("%s supports %d property set%s", obj_name, propsets_len, (propsets_len != 1) ? "s" : "");
...
gboolean gst_ks_video_device_open(GstKsVideoDevice *self)
{
     ...
     * Open the filter.
     */
    priv->filter_handle = CreateFile(priv->dev_path, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
     ...
     */
    if (ks_object_get_supported_property_sets(priv->filter_handle, &propsets, &propsets_len)) {
        gst_ks_video_device_dump_supported_property_sets(self, "filter", propsets, propsets_len);
        g_free(propsets);
    } else {
    ...
    for (cur = priv->media_types; cur != NULL; cur = cur->next) {
        ...
        gst_caps_append(priv->cached_caps, gst_caps_copy(media_type->translated_caps));
...
static HANDLE gst_ks_video_device_create_pin(GstKsVideoDevice *self, KsVideoMediaType *media_type,
                                             gulong *num_outstanding)
{
    GstKsVideoDevicePrivate *priv = GST_KS_VIDEO_DEVICE_GET_PRIVATE(self);
     ...
     */
    if (ks_object_get_supported_property_sets(pin_handle, &propsets, &propsets_len)) {
        guint i;

        gst_ks_video_device_dump_supported_property_sets(self, "pin", propsets, propsets_len);
    ...
    *num_outstanding = 0;
    ...
    if (ks_object_get_property(pin_handle, KSPROPSETID_Connection, KSPROPERTY_CONNECTION_ALLOCATORFRAMING_EX,
                               &framing_ex, NULL)) {
        ...
        if (ks_object_get_property(pin_handle, KSPROPSETID_Connection, KSPROPERTY_CONNECTION_ALLOCATORFRAMING, &framing,
                                   &framing_size)) {
    ...
    GST_DEBUG("num_outstanding: %d alignment: 0x%08x", *num_outstanding, alignment);
    ...
    if (supports_mem_transport) {
        ...
        if (!ks_object_set_property(pin_handle, KSPROPSETID_MemoryTransport, KSPROPERTY_MEMORY_TRANSPORT,
                                    &mem_transport, sizeof(mem_transport))) {
    ...
    if (priv->clock != NULL) {
        ...
        if (ks_object_get_property(pin_handle, KSPROPSETID_Stream, KSPROPERTY_STREAM_MASTERCLOCK,
                                   (gpointer *)&cur_clock_handle, &cur_clock_handle_size)) {
            ...
            if (ks_object_set_property(pin_handle, KSPROPSETID_Stream, KSPROPERTY_STREAM_MASTERCLOCK, &new_clock_handle,
...
static void gst_ks_video_device_close_current_pin(GstKsVideoDevice *self)
{
    GstKsVideoDevicePrivate *priv = GST_KS_VIDEO_DEVICE_GET_PRIVATE(self);
...
gboolean gst_ks_video_device_set_caps(GstKsVideoDevice *self, GstCaps *caps)
{
    ...
    if (!gst_caps_is_subset(caps, priv->cached_caps)) {
        ...
        GST_ERROR("caps (%s) is not a subset of device caps (%s)", string_caps, string_c_caps);
    ...
    if (!ks_video_fixate_media_type(media_type->range, media_type->format, width, height, fps_n, fps_d))
        goto error;

    if (priv->cur_media_type != NULL) {
        if (media_type->format_size == priv->cur_media_type->format_size
            && memcmp(media_type->format, priv->cur_media_type->format, priv->cur_media_type->format_size) == 0) {
    ...
    pin_handle = gst_ks_video_device_create_pin(self, media_type, &priv->num_requests);
    if (!ks_is_valid_handle(pin_handle)) {
        /* Re-create the old pin */
        if (priv->cur_media_type != NULL)
            priv->pin_handle = gst_ks_video_device_create_pin(self, priv->cur_media_type, &priv->num_requests);
        goto error;
    }
...
gboolean gst_ks_video_device_set_state(GstKsVideoDevice *self, KSSTATE state)
{
    ...
    GST_DEBUG("Initiating pin state change from %s to %s", ks_state_to_string(priv->state), ks_state_to_string(state));
    ...
    while (priv->state != state) {
        ...
        GST_DEBUG("Changing pin state from %s to %s", ks_state_to_string(priv->state), ks_state_to_string(next_state));
        ...
        if (ks_object_set_connection_state(priv->pin_handle, next_state)) {
            ...
            GST_WARNING("Failed to change pin state to %s", ks_state_to_string(next_state));
            return FALSE;
        }
    }

    GST_DEBUG("Finished pin state change from %s to %s", ks_state_to_string(initial_state), ks_state_to_string(state));
...
gboolean gst_ks_video_device_get_latency(GstKsVideoDevice *self, GstClockTime *min_latency, GstClockTime *max_latency)
{
    ...
    *min_latency = gst_util_uint64_scale_int(GST_SECOND, priv->fps_d, priv->fps_n);
    *max_latency = *min_latency;

    return TRUE;
}

static gboolean gst_ks_video_device_request_frame(GstKsVideoDevice *self, ReadRequest *req, gulong *error_code,
                                                  gchar **error_str)
{
    GstKsVideoDevicePrivate *priv = GST_KS_VIDEO_DEVICE_GET_PRIVATE(self);
    HANDLE                   event;
    ...
    success = DeviceIoControl(priv->pin_handle, IOCTL_KS_READ_STREAM, NULL, 0, params, params->header.Size,
                              &bytes_returned, &req->overlapped);
    ...
    /* ERRORS */
error_ioctl : {
    gst_ks_video_device_parse_win32_error("DeviceIoControl", GetLastError(), error_code, error_str);
    return FALSE;
}
}
                                             ...
                                             GstClockTime *presentation_time, gulong *error_code, gchar **error_str)
{
    ...
    do {
        ...
        wait_ret = WaitForMultipleObjects(priv->request_events->len, (HANDLE *)priv->request_events->data, FALSE,
                                          READ_TIMEOUT);
        ...
        *bytes_read = 0;
        ...
        for (req_idx = wait_ret - WAIT_OBJECT_0; req_idx < priv->num_requests; req_idx++) {
            ReadRequest *req = &g_array_index(priv->requests, ReadRequest, req_idx);
            ...
            success = GetOverlappedResult(priv->pin_handle, &req->overlapped, &bytes_returned, TRUE);
            ...
            if (success) {
                ...
                *bytes_read = hdr->DataUsed;
                ...
                if (G_UNLIKELY(GST_DEBUG_IS_ENABLED())) {
                    gchar *options_flags_str = ks_options_flags_to_string(hdr->OptionsFlags);

                    GST_DEBUG("PictureNumber=%" G_GUINT64_FORMAT ", DropCount=%" G_GUINT64_FORMAT
                              ", PresentationTime=%" GST_TIME_FORMAT ", Duration=%" GST_TIME_FORMAT
                              ", OptionsFlags=%s: %d bytes",
                              frame_info->PictureNumber, frame_info->DropCount, GST_TIME_ARGS(timestamp),
                              GST_TIME_ARGS(duration), options_flags_str, hdr->DataUsed);
                ...
                if (G_LIKELY(GST_CLOCK_TIME_IS_VALID(timestamp))) {
                    if (G_UNLIKELY(GST_CLOCK_TIME_IS_VALID(priv->last_timestamp) && timestamp < priv->last_timestamp)) {
                        GST_WARNING("got an old frame (last_timestamp=%" GST_TIME_FORMAT ", timestamp=%" GST_TIME_FORMAT
                                    ")",
                                    GST_TIME_ARGS(priv->last_timestamp), GST_TIME_ARGS(timestamp));
                        *bytes_read = 0;
                    } else {
                ...
                if (*bytes_read > 0) {
                    ...
                    if (priv->is_mjpeg) {
                        ...
                        /* JFIF SOI marker */
                        if (*bytes_read > MJPEG_MAX_PADDING && buf[0] == 0xff && buf[1] == 0xd8) {
...
error_wait : {
    gst_ks_video_device_parse_win32_error("WaitForMultipleObjects", GetLastError(), error_code, error_str);
...
error_get_result : {
    gst_ks_video_device_parse_win32_error("GetOverlappedResult", GetLastError(), error_code, error_str);
